                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 text-end">
                                    ...
                                    <address>
                                        ...
                                        Địa chỉ: <strong>
                                            {{ $order->address }}, {{ $order->ward->name_with_type }},
                                            {{ $order->district->name_with_type }}, {{ $order->province->name_with_type }}
                                        </strong><br>
                        ...
                        <div class="card-body">
                            <form action="{{ route('admin.order.update', $order->id) }}" method="POST">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="payment_method" id="payment_method" class="form-select" disabled>
                                        @foreach ($paymentMethod as $key => $status)
                                            <option value="{{ $key }}"
                                                @if ($order->transaction->payment_method == $key) selected @endif>
                                                {{ $status }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="payment_status" id="payment_status" class="form-select" disabled>
                                        @foreach ($paymentStatus as $key => $status)
                                            <option value="{{ $key }}"
                                                @if ($order->transaction->payment_status == $key) selected @endif>
                                                {{ $status }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <a class="nav-link text-primary mt-2" href="{{ route('admin.transaction.index') }}">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="shipping_method" id="shipping_method" class="form-select" disabled>
                                        @foreach ($shippingMethod as $key => $status)
                                            <option value="{{ $key }}"
                                                @if ($order->shipping->shipping_method == $key) selected @endif>
                                                {{ $status }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="shipping_status" id="shipping_status" class="form-select" disabled>
                                        @foreach ($shippingStatus as $key => $status)
                                            <option value="{{ $key }}"
                                                @if ($order->shipping->shipping_status == $key) selected @endif>
                                                {{ $status }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <a class="nav-link text-primary mt-2" href="{{ route('admin.shipping.index') }}">
